...
class AuthBloc extends Bloc<AuthEvent, AuthState> {
  ...
  Future<void> _onLogin(LoginEvent event, Emitter<AuthState> emit) async {
    ...
    try {
      ...
      if (authModel.token != null && authModel.token.isNotEmpty) {
        ...
        _logger.logWarning('Login failed: empty token');
      }
    } catch (e, stack) {
  ...
  Future<void> _onRegister(RegisterEvent event, Emitter<AuthState> emit) async {
    _logger.logInfo('Register started');
    emit(AuthLoading());
    try {
      ...
      if (authModel.token != null && authModel.token.isNotEmpty) {
        emit(AuthSuccess());
        _logger.logInfo('Register successful');
      } else {
        emit(AuthError('Register failed: empty token received'));
        _logger.logWarning('Register failed: empty token');
      }
    } catch (e, stack) {
      AuthErrorResponseModel? errorResponse;
      try {
  ...
  Future<void> _onSignOut(SignOutEvent event, Emitter<AuthState> emit) async {
    _logger.logInfo('Sign out started');
    emit(AuthLoading());
    try {
      await authService.signOut();
      emit(AuthInitial());
      _logger.logInfo('Sign out successful');
    } catch (e, stack) {
      emit(AuthError('Sign out failed: ${e.toString()}'));
      _logger.logError('Sign out error',
          error: e is Exception ? e : Exception(e.toString()), stack: stack);
    }
  }
  ...
  void _onCheckAuthStatus(
    ...
    _logger.logInfo('Auth status check started');
    emit(AuthLoading());
    try {
      final hasToken = await authService.isAuthenticated();
      if (hasToken) {
        emit(AuthSuccess());
        _logger.logInfo('Auth token found');
      } else {
        emit(AuthInitial());
        _logger.logInfo('No auth token found');
      }
    } catch (e, stack) {
      emit(AuthError('Token check failed: ${e.toString()}'));
      _logger.logError('Auth status check error',
          error: e is Exception ? e : Exception(e.toString()), stack: stack);
    }
  }
}
